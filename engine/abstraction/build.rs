...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Block 2.5 fix: tonic-build 0.11's builder method is `compile`
    // (`compile_protos` does not exist); the old call never compiled because
    // the crate had no working toolchain until now.
